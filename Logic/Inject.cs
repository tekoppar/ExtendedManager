...
namespace GijSoft.DllInjection {
    ...
    public sealed class DllInjector {
        ...
        static extern bool EnumProcessModules(IntPtr hProcess, [Out] IntPtr[] lphModule, uint cb, out uint lpcbNeeded, uint dwFilterFlag);
        ...
        public DllInjectionResult InjectA(string sProcName, string sDllPath) {
            if (!File.Exists(sDllPath)) {
                return DllInjectionResult.DllNotFound;
            }
        ...
        public DllInjectionResult Unload(string sProcName, string sDllPath) {
            if (!File.Exists(sDllPath)) {
                return DllInjectionResult.DllNotFound;
            }
        ...
        bool bInjectA(uint pToBeInjected, string sDllPath) {
            IntPtr hndProc = OpenProcess((0x2 | 0x8 | 0x10 | 0x20 | 0x400), 1, pToBeInjected);
            ...
            int writtenBytes = 0;
            if (WriteProcessMemory(hndProc, lpAddress, bytes, (uint)bytes.Length, ref writtenBytes) == 0) {
                return false;
            }
        ...
        public bool bModuleLoaded(string sProcName, string moduleName) {
            bool results = false;
            ...
            if (!EnumProcessModules(hndProc, buffer, cb, out uint totalModules, 3u)) {
                return false;
            }
            ...
            while ((long)count < (long)((ulong)moduleSize)) {
                if (GetModuleBaseName(hndProc, buffer[count], stringBuilder, (uint)stringBuilder.Capacity) == 0u) {
                    return false;
                }
        ...
        bool bUnload(uint pToBeInjected, string sDllPath) {
            IntPtr hndProc = OpenProcess((0x2 | 0x8 | 0x10 | 0x20 | 0x400), 1, pToBeInjected);
